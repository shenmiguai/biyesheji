%% 这段代码主要用于对一个加速度信号（或其他时序数据）进行频域分析，并计算不同频带的振动信号能量。
% 这段代码在整体工程中起到了以下作用：
% 频域分析：通过FFT计算信号的频域表示，并展示了信号的频谱。
% 功率谱密度分析：通过计算并绘制功率谱密度（PSD），展示了信号在不同频率上的功率分布。
% 三分之一倍频程分析：计算了各个频带（通过三分之一倍频程定义）内的信号能量，提供了更精细的频域特征。
% 信号能量评估：通过RMS值评估每个频带的能量，从而更好地理解信号在不同频率范围内的强度。
% 这段代码可以应用于振动信号、音频信号等领域的频域分析，用于信号的特征提取、信号处理和故障诊断等。
%%
clear;clc;clf;
