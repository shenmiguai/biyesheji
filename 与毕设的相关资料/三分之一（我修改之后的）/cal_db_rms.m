...
data_dir = './data/加速度1.xlsx';% 指定要读取的数据文件路径
...
matrixs = Signal(:,2:end);    % 后面的列是加速度信号（可以有多通道）

%% 判断是否需要积分_三分之一倍频程分析 + dB计算
%把每个通道的振动信号分成一堆频段，用FFT做带通滤波，计算每个频段的RMS（能量强度），然后转换成dB（响度大小）做特征提取。

[a, b] = size(matrixs);              % 获取数据矩阵的尺寸，a是采样点数，b是通道数
L_db_Matrix = [];                    % 初始化一个空矩阵，用来存储每个通道的dB值结果

for i = 1:b                          % 对每个通道数据进行处理
    channel_value = matrixs(:,i);   % 取出第i个通道的数据
    fs = 1/dt;                       % 根据时间间隔计算采样频率
    f = f_zhong;                     % 中心频率数组（1/3倍频程的标准频率）
    fc = [f 10*f 100*f 1000*f];      % 扩展频率范围，包括原频率及其10倍、100倍、1000倍
    oc6 = 2^(1/6);                   % 每个频带上下限的倍数因子（1/6倍频程）
    nc = length(fc);                % 总共的频带数量
    n = length(channel_value);      % 当前通道的数据长度（采样点数量）
...
%20*log10(RMS/参考值)就是标准的声压级公式
    ...
    L_db_Matrix = [L_db_Matrix; L_db]; % 将当前通道的dB值加入结果矩阵中
...
mat_w_db = [fc(1:size(L_db_Matrix,2))' db_mean]; % 加上频率列
