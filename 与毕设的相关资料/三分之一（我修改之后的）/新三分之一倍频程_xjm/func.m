...
%读取excel文件
            ...
            t=0:1/fs:(n-1)/fs;
            plot(app.TIMECH,t,y1);drawnow % plot(t,x);
...
%---------------------------功率谱-------------------------------
 ...
 %           fs_fft=44100; %添加一个采样频率输入框  %40k soundfs=44100
            
            ff=linspace(0,fs/2,ceil(n/2));%ff=ff'; %x轴
          ...
          %功率谱 
           % plot(app.Power,log10(ff),Power(1:ceil(n/2)));   
           
%---------------------添一点功能--------------------------------
            app.Average.Value=mean(x); %在文本框显示 %每一个时间段给一个值
            app.RMS.Value=rms(x);
            app.Peak.Value=max(x);
            app.VPP.Value=peak2peak(x);
%-----------------------复选框-------------------------------------            
             switch value %复选框
                case('功率谱')
                    plot(app.FANALYSIS,log10(ff),Power(1:ceil(n/2)));%db(功率谱)
                case('幅频')
                    plot(app.FANALYSIS,log10(ff),Amplitude(1:ceil(n/2)));%x轴hz  y轴Y
                case('相频')
                    ph = angle(a)*180/pi;
                    plot(app.FANALYSIS,log10(ff),ph(1:ceil(n/2)));%x轴hz  y轴Y
                case('实频')
                    r=real(a);
                    plot(app.FANALYSIS,log10(ff),r(1:ceil(n/2)));%x轴hz  y轴Y
                case('虛频')
                    i=imag(a);
                    plot(app.FANALYSIS,log10(ff),i(1:ceil(n/2)));%x轴hz  y轴Y
            end
            drawnow;           
...
%------------------------1/3倍频RMS(两种坐标)、A声级-------------------------------------                   
        bar(app.ONETHIRD_1_2_3_4,yc(1:m));drawnow;%绘制1/3分之一倍频的bar_RMS值，x坐标是1，2，3，4，5....
        bar(app.ONETHIRD_fc,fc(1:m),yc(1:m)); drawnow;%以三分之一倍频的中心频率为坐标   
        plot(app.Ashengji,fc(14:m),yc(14:m));  %%绘制A声级 %xscale','log','xtick',fc,'XLim',[20 20000] %xlabel('频率(Hz)');ylabel('A声级(dB)');
...
% %-----------------------------绘制bar_RMS值---------------------------------
% figure  
% bar(yc(1:m));%绘制bar_RMS值，x坐标是1，2，3，4，5....
% grid on
% xlabel('三分之一倍频中心频率序号');
% ylabel('RMS值');
% title('有效值RMS');
...
% %--------------三分之一倍频每一中心频率的RMS值------------------------
% figure 
% bar(fc(1:m),yc(1:m)); %以三分之一倍频的中心频率为坐标
% grid minor;
% %grid on;
...
% %------------------------绘制A声级-------------------------------------
%  %使用这个 yc=10*log10(yc/1e-10); %yc=(1x60) %使用这个
% %%
