%---------------------------------------------------------
%A声级，等效连续A声级，倍频带声压级
clear;
clc;close all;
%输入需要分析的时程数据
s = load('E:\matlab2021aa\sum_proj\三分之一倍频\新三分之一倍频程_xjm/sudu_two_column.mat');
t=s.matrixs(:,1); %针对一列数据  %时间
x=s.matrixs(:,2);%速度
subplot 211
plot(t,x);%原始数据波形
grid on;

subplot 212
plot(x);%原始数据波形
grid on;

% fs =524288;      %采样频率，大于40kHz等效A声级数据准确，大于2000Hz倍频带声压级可运行；
fs=400000;%xjm

% f=[1.00 1.25 1.60 2.00 2.50 3.15 4.00 5.00 6.30 8.00];%定义1/3倍频程中心频率
% fc=[f,10*f,100*f,1000*f,10000*f,100000*f]; %fcmax=80w

fc= [1 1.25 1.6 2 2.5 3.15 4 5 6.3 8 10 12.5 16 20 25 31.5 40 50 63 80 100 125 160 200 250 315 400 500 630 800 1000 1250 1600 2000 2500 3150 4000 5000 6300 8000 10000 12500 16000 20000];
%xjm   fc=1~20khz

oc6=2^(1/6);%中心频率与下限频率的比值 %oc6=1.1225
nc=length(fc);%取中心频率总的长度  %nc=44
n=length(x);%输入数据的长度 %n=2048
nfft=2^nextpow2(n);%大于并接近n的2的幂次方长度  %2048
a=fft(x,nfft);%FFT变换  a=(2048x1) % a复数
yc = zeros(1,nc);
for j=1:nc %nc=44
    fl=fc(j)/oc6; 
    fu=fc(j)*oc6;  %fc=20khz对应fl=224480hz
    nl=round(fl*nfft/fs+1); %下限频率对应的序号
    nu=round(fu*nfft/fs+1);%上限频率对应的序号  
    if fu>fs/2 %如果上限频率大于半谱频率则循环中断
        m=j-1; disp(m);
        break  %判断有效的频率，当一旦出现频率值超过fs/2时，立即停止后边数据的检查，因为出现了f太大的信号，不符合奈奎斯特定律     
    end
    disp(j);     
    b=zeros(1,nfft);
    b(nl:nu)=a(nl:nu);
    b(nfft-nu+1:nfft-nl+1)=a(nfft-nu+1:nfft-nl+1);%以每个中心频率段为通带进行带通频域滤波
    c=ifft(b,nfft);%c是复数  
    yc(j)=sqrt(var(real(c(1:n)))); %该公式认为信号c是零均值的  %yc=RMS %每个中心频率段的RMS 	%计算对应每个中心频率段的有效值%注意，matlab函数var和std认为操作数是度量总体的一个样本，所以使用的公式是实际值与期望值之差的平方和再除以（N-1）
end
yc=10*log10(yc/1e-10); %yc=(1x60) 
%yc = 20 * log10(yc / (20e-5));   % 严格标准写法                
% yc = 20 * log10(yc /1 * 10^(-6));  % 参考加速度（通常是1微米/秒??）

%A计权
% cf=zeros(1,m);% cf=(1x54)
% cf(1,11:44)=[-70.4,-63.4,-56.7,-50.5,-44.7,-39.4,-34.6,-30.2,-26.2,-22.5,-19.1,-16.1,-13.4,-10.9,-8.6,...
%     -6.6,-4.8,-3.2,-1.9,-0.8,0,0.6,1.0,1.2,1.3,1.2,1.0,0.5,-0.1,-1.1,-2.5,-4.3,-6.6,-9.3]; 
% %-70.4对应10HZ  % 20Hz~40khz  %-2.5对应20khz
% yc=yc(1:m)+cf(1:m);

%xjm
cf1=zeros(1,m);
cf1(1,11:41)=[-70.4,-63.4,-56.7,-50.5,-44.7,-39.4,-34.6,-30.2,-26.2,-22.5,-19.1,-16.1,-13.4,-10.9,-8.6,...
    -6.6,-4.8,-3.2,-1.9,-0.8,0,0.6,1.0,1.2,1.3,1.2,1.0,0.5,-0.1,-1.1,-2.5];%20~20khz
yc1=yc(1:m)+cf1(1:m);

%%                                                                                                                                                                                                      
%-----------------------绘制输入时程曲线图形-----------------------------
figure 
t=0:1/fs:(n-1)/fs;
plot(t,x);
xlabel('时间(s)');
ylabel('加速度(m/s^2)');
grid on;
title('输入时程曲线图形');

%------------------------绘制A声级-------------------------------------
figure;
plot(fc(14:m),yc(14:m),'r-s','LineWidth',1.5);          %从20Hz开始画，因此取14。20hz是第十四个频率
xlabel('频率(Hz)');
ylabel('A声级(dB)');
grid on;
set(gca,'xscale','log','xtick',fc,'XLim',[20 20000],'Fontname','宋体','Fontsize',12);
set(gcf,'Position',[250,200,800,500]);
sa=yc(1,14:44);
save('006.txt','sa','-ascii' );
title('绘制A声级');
%-------------------------------------
figure;%绘制bar
bar(yc(14:m)); % 调整柱宽

%% -----------------------连续等效A声级的值------------------------------------
% if fc(1,m)>=20000
%     i_num=44-14+1;
% else
%     i_num=m-14+1;
%     disp('频率区间未到20kHz，等效连续A声级计算不准确！！！');
% end
% L=[];LeqA=0;  
% L=yc(1,14:44);
% for i=1:i_num    
%    LeqA=LeqA+10^(0.1*L(1,i));       %L(1,i)是不是代表第一行，所有列
% end
% LeqA=10*log10(LeqA);
% L=sprintf('等效连续A声级为：%.2f',LeqA);
% disp(L);



