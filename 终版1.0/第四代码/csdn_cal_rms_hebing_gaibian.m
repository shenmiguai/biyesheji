...
%%
% 去除1次趋势项
[y1, xtrend] = polydetrend(sig, fs, 1); %自定义函数
figure;
subplot(3, 1, 1); plot(t,sig); title('原始信号');grid on;
subplot(3, 1, 2); plot(t,y1); title('去除非线性趋势项后信号(polyfit拟合非线性方法)');grid on;
subplot(3, 1, 3); plot(t,xtrend); title('非线性趋势项'); grid on;
x=y1;%x是去趋势项之后的加速度数据
...
%% 同效果
% 2. 去除线性趋势项   
% [p, s, mu] = polyfit(t,sig, 1);%拟合系数p  %matlab自带函数
% figure;
% subplot(4, 1, 1); plot(t,sig); title('原始信号');
% subplot(4, 1, 2); plot(t,detrend(sig)); title('去除线性趋势项的原始信号(detrend去除线性趋势项方法)');
% subplot(4, 1, 3); plot(t,polyval(p,t)); title('线性趋势项（polyfit拟合直线）');% 画的是线性趋势项本身
% % mu(1) 是 mean(x)均值，mu(2) 是 std(x)方差
% %  画出用 polyfit 拟合出来的趋势线。
% %  detrend(sig)等效于从 sig 中减去最优线性拟合直线，去除整体上升或下降的趋势。
% %s：拟合的统计量（通常不用)。
% %mu：数据中心化时的均值和标准差，用于数值稳定性（通常不用，除非需要精确拟合误差分析）。